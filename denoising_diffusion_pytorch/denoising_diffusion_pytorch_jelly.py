...
class Decode(nn.Module):
	def __init__(self):
		...
		self.decode = nn.Sequential(nn.Conv2d(21, 21, 3, stride=2), nn.ELU(alpha=1.0,inplace=False), 
									nn.Conv2d(21, 21, 5, stride=3), nn.ELU(alpha=1.0,inplace=False),
									nn.Conv2d(21, 21, 3, stride=2), nn.ELU(alpha=1.0,inplace=False))
		self.l1 = nn.Linear(1, 2)
		# self.decode = nn.Sequential(nn.Conv2d(3, 3, 7, stride=2), nn.ELU(alpha=1.0,inplace=False), 
		#                             nn.Conv2d(3, 3, 9, stride=3), nn.ELU(alpha=1.0,inplace=False),
		# 	                        nn.Conv2d(3, 1, 7, stride=2), nn.ELU(alpha=1.0,inplace=False))
	def forward(self, x):
		# B X 20 X 32 X 32
		#
		x = self.decode(x)
...
#        assert not (type(self) == GaussianDiffusion and model.channels != model.out_dim)
		...
		# self.class_linear = nn.Linear(1, self.image_size*self.image_size).cuda()
	...
	def model_predictions(self, x, t, x_self_cond = None, clip_x_start = False, rederive_pred_noise = False):
		...
		if self.objective == 'pred_noise':
			...
			# model_output_0, middle_out_0  = self.model(x_start, t, x_self_cond)
			sample_mask = self.modelz(middle_out, x_start, t, x_self_cond)

			mask1 = torch.clamp(sample_mask[:, :self.channels, :, :], min=0.0, max=1.0)  # B X 20 X 32 X 32
			sample_mask = torch.cat((mask1.expand(-1, self.channels, -1,-1), sample_mask[:, self.channels:, :, :].expand(-1, self.channels, -1,-1)), dim=1)  
	...
	def p_sample(self, x, t: int,  x_self_cond = None):
		...
		sample_mask = torch.clamp(sample_mask[:,:3], min=0.0, max=1.0)
		pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
	...
	def p_sample_loop(self, shape, return_all_timesteps = False):
		...
		# img = 0.01*torch.randn(x.shape, device = device) + 1.5*torch.randn(x.shape, device = device)
	...
	def p_losses(self, x_start, t, area, noise = None, offset_noise_strength = None):
		...
		# model_out_0, middle_out_0 = self.model(x_start, t, x_self_cond)  # B X 20 X 32 X 32
		mask = self.modelz(middle_out, x_start, t, x_self_cond)  # B X 40 X 32 X 32
		
		mask1 = torch.clamp(mask[:, :self.channels, :, :], min=0.0, max=1.0)  # B X 20 X 32 X 32
		...
		if self.objective == 'pred_noise':
			target = noise.clone() *(mask[:,:self.channels,:]+mask[:,self.channels:,:])/2
		elif self.objective == 'pred_x0':
			target = x_start
		elif self.objective == 'pred_v':
...
class MyDataset(Dataset):
	def __init__(self,train):
		if train:
			...
			# self.data,self.label = load_data('/zhangqianru/data/Cifar/train_data.pickle')[0], load_data('/zhangqianru/data/Cifar/train_data.pickle')[1]
			...
			#train_dog_cat_32.pickle
			# self.data,self.label = load_data('/zhangqianru/data/train_dog_cat_32.pickle')[0], load_data('/zhangqianru/data/train_dog_cat_32.pickle')[1]
...
class Trainer(object):
	def __init__(
		...
		*,
		...
		# self.load_pretrain('/zhangqianru/code/results_cele_v5_logits/model-14.pt')
