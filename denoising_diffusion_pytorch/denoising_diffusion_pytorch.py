...
#        assert not (type(self) == GaussianDiffusion and model.channels != model.out_dim)
	...
	#@torch.inference_mode()
	@torch.no_grad()
	def p_sample_loop(self, shape, return_all_timesteps = False, ground_truth = None):
		...
		if ground_truth == None:
			...
			p_t = torch.tensor([999]*shape[0],device=device)
			ground_truth = ground_truth.to(device)
			img, _ = self.q_sample(x_start = ground_truth, t = p_t)
	...
	# @torch.inference_mode()
	@torch.no_grad()
	def ddim_sample(self, shape, return_all_timesteps = False, ground_truth = None):
	...
	#@torch.inference_mode()
	@torch.no_grad()
	def sample(self, batch_size = 16, return_all_timesteps = False, ground_truth = None):
		image_size, channels = self.image_size, self.channels
		sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
		return sample_fn((batch_size, channels, image_size, image_size), return_all_timesteps = return_all_timesteps, ground_truth = ground_truth)
